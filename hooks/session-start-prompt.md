...
- Si sa demande est exactement `/zenvibe:resume` → ignore cette consigne, le skill se charge déjà du briefing complet.
...
Briefing complet : `/zenvibe:resume`
```
...
- Lis **uniquement** la dernière entrée du JOURNAL. Pas de `git status`, pas de `CLAUDE.md` — ces lectures appartiennent à `/zenvibe:resume`.
...
- Le mini-briefing est un *teaser*, pas un substitut à `/zenvibe:resume`. Ne le développe pas.
- Ne propose pas spontanément de reprendre la tâche — laisse l'utilisateur conduire. S'il veut reprendre, il lancera `/zenvibe:resume`.
